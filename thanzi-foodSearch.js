...
(function (global) {
  ...
  /**
   * Parse a grams value out of a free-text serving/quantity string.
   * Handles forms like "35 g", "1 pack (250g)", "250ml", "1.5 oz (42g)".
   * Returns null if no gram figure can be extracted.
   */
  function _parseGramsFromText(str) {
    if (!str || typeof str !== 'string') return null;
    // Prefer a number that's explicitly followed by g/gram/gramme (not mg/kg)
    const m = str.match(/(\d+(?:[.,]\d+)?)\s*(?:g|gram|grammes?)\b(?!\w)/i);
    if (m) return parseFloat(m[1].replace(',', '.'));
    // Fall back to ml (treat 1ml ≈ 1g for liquids, close enough for portioning)
    const mlMatch = str.match(/(\d+(?:[.,]\d+)?)\s*ml\b/i);
    if (mlMatch) return parseFloat(mlMatch[1].replace(',', '.'));
    return null;
  }

  /** Build a household-measures entry for "1 pack/serving (Xg)" from label data */
  function _labelMeasures({ servingText, servingGrams, packGrams, unit }) {
    const out = [];
    const u = unit === 'mL' ? 'mL' : 'g';

    if (servingGrams && servingGrams > 0) {
      const label = servingText && !/^\d/.test(servingText.trim())
        ? servingText.trim()
        : `1 serving`;
      out.push({ label: `${label} (${Math.round(servingGrams)}${u})`, g: servingGrams });
    }
    if (packGrams && packGrams > 0 && packGrams !== servingGrams) {
      out.push({ label: `1 pack (${Math.round(packGrams)}${u})`, g: packGrams });
    }
  ...
  async function _searchMWBarcode(barcode) {
    try {
      ...
      const measures = _labelMeasures({
        servingText:  d.serving_size || d.serving_label || null,
        servingGrams: d.serving_size_g ?? d.serving_g ?? _parseGramsFromText(d.serving_size),
        packGrams:    d.package_weight_g ?? d.net_weight_g ?? _parseGramsFromText(d.package_weight || d.net_weight),
        unit: 'g',
      });
      return {
  ...
  function _offShape(p, barcode = null) {
    ...
    const unit = _offUnit(p);
    const measures = _labelMeasures({
      servingText:  p.serving_size || null,
      servingGrams: _parseGramsFromText(p.serving_size),
      packGrams:    _parseGramsFromText(p.product_quantity ? p.product_quantity + ' g' : p.quantity),
      unit,
    });
    return {
  ...
  async function _searchOFF(query) {
    try {
      const url = 'https://world.openfoodfacts.org/cgi/search.pl'
        ...
        + 'food_groups_tags,nutrition_data_per,product_quantity_unit,'
        + 'serving_size,product_quantity,quantity,nutriments';
      const res = await fetch(url, {
        signal:  _signal(8000),
        headers: { Accept: 'application/json' },
      });
